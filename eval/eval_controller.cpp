...
namespace
{
        ...
        std::vector<std::uint64_t> const& reference_values) -> std::string
    {
        ...
        for (auto value : sph_values) maximum = std::max(maximum, value);
        for (auto value : reference_values) maximum = std::max(maximum, value);
        ...
        output << "```mermaid\nxychart-beta\n"
            ...
            << "    bar " << values(sph_values) << "\n"
            << "    bar " << values(reference_values) << "\n"
            << "```\n\n";
        return output.str();
    }
    ...
    auto make_report(std::vector<level_results> const& results, std::uint64_t iterations) -> std::string
    {
        ...
        {
            sph_compressed_size.push_back(result.sph_compress.output_bytes);
            reference_compressed_size.push_back(result.reference_compress.output_bytes);
        ...
        output << report_begin << "\n\n## Current evaluation\n\n"
            ...
            << "mixed corpus. In every chart the **first bar series is sph-zstd++** and the **second is "
            << "reference zstd**. Code size is the complete worker executable; memory is peak resident set "
            ...
            << "### Compression\n\n";
        output << chart("Compressed size", "bytes", sph_compressed_size, reference_compressed_size);
